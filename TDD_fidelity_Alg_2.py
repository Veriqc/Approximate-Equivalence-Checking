...
def Simulation_with_tree_decomposition(cir, num_qubit, scheme=2, connect_in_and_out=False, ini_tdd=True):
...
def Simulation_with_time_out(cir, num_qubit,scheme=2,connect_in_and_out=False,ini_tdd=True):
    try:
...
def assign_noisy(cir, the_noisy_position, p):
    """
    add noises into the circuit
    """
    total_noisy_cir = nx.DiGraph()
    ...
    for k in cir.nodes:
        ...
        total_noisy_cir.add_node(total_gate_num)
        total_noisy_cir.nodes[total_gate_num]['operation'] = operation
        total_gate_num += 1
        total_noisy_cir.add_node(total_gate_num)

        if operation.name == 'CX':
            new_operation = OperationCNOT(operation.control_qubit + num_qubit, operation.target_qubit + num_qubit)
        elif operation.name in ['swap', 'cul', 'two']:
            new_operation = OperationTwo(
                [operation.involve_qubits_list[0] + num_qubit, operation.involve_qubits_list[1] + num_qubit],
                u_matrix=operation.u_matrix.conjugate())
        else:
            new_operation = OperationSingle(operation.involve_qubits[0] + num_qubit, operation.u_matrix.conjugate(),
                                            operation.name)
        total_noisy_cir.nodes[total_gate_num]['operation'] = new_operation
        total_gate_num += 1

        if k in the_noisy_position:
            model, E, M = noisy_model(p, model=3)
            #             model,E,M = real_noisy_model(nam,q)
            q = [q[0]]
            for noisy_qubit in q:
                new_operation = OperationU([noisy_qubit, noisy_qubit + num_qubit], 'noisy', M)
                total_noisy_cir.add_node(total_gate_num)
                total_noisy_cir.nodes[total_gate_num]['operation'] = new_operation
                total_gate_num += 1
                noisy_gate_num += 1
    ...
    print('noisy_gate_num:', noisy_gate_num)
    print('the_noisy_model:', the_noisy_model)
    return total_noisy_cir, noisy_gate_num
...
def get_miter(cir,noisy_cir):
    ...
    for k in range(len(cir.nodes)-1,-1,-1):
        operation = cir.nodes[k]['operation']
        miter_cir.add_node(gate_num)
        miter_cir.nodes[gate_num]['operation'] = copy.copy(operation)
        if miter_cir.nodes[gate_num]['operation'].name !='CX':
            miter_cir.nodes[gate_num]['operation'].u_matrix=miter_cir.nodes[gate_num]['operation'].u_matrix.conjugate().T
        gate_num+=1
        miter_cir.add_node(gate_num)
        if operation.name == 'CX':
            new_operation = OperationCNOT(operation.control_qubit+num_qubit,operation.target_qubit+num_qubit)
        elif operation.name in ['swap','cul','two']:
            new_operation = OperationTwo([operation.involve_qubits_list[0]+num_qubit,operation.involve_qubits_list[1]+num_qubit], u_matrix=operation.u_matrix.T)
        else:
            new_operation = OperationSingle(operation.involve_qubits[0]+num_qubit,operation.u_matrix.T,operation.name)
        miter_cir.nodes[gate_num]['operation'] = new_operation
...
if __name__=="__main__":
    ...
    total_noisy_cir, _ = assign_noisy(dag_cir1, noisy_position, 0.001)
    miter_cir = get_miter(dag_cir1, total_noisy_cir)
    num_qubit_miter = get_real_qubit_num(miter_cir)
    print('-------------')

    t_start = time.time()
    tdd, max_node_num, tree_width = Simulation_with_time_out(miter_cir, num_qubit_miter, scheme=2, connect_in_and_out=True)
